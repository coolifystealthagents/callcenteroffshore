...
export default async function Service({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <><Header /><main><JsonLd data={schema} />
    <section className="page-hero service-detail-hero"><div className="container page-hero-grid"><div><p className="cco-kicker"><span /> {service.eyebrow}</p><h1>{service.title}</h1><p className="lead">{service.desc}</p><div className="cco-actions"><a className="cco-btn cco-btn-primary" href="/contact-us">Discuss this role <span>↗</span></a><a className="cco-btn cco-btn-ghost" href="/services">All services</a></div></div><div className="page-hero-art"><img src={service.image} alt={`Filipina professional illustrating ${service.title.toLowerCase()}`} /></div></div></section>
