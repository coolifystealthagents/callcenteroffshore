...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  const dated = (aug23Meta as Record<string, {title:string;description:string;hero:string}>)[slug];
  if (dated) { const canonical=`https://callcenteroffshore.com/blog/${slug}`; return { title: dated.title, description: dated.description, alternates: { canonical }, openGraph: { title: dated.title, description: dated.description, url: canonical, type: 'article', images: [{ url: `https://callcenteroffshore.com${dated.hero}`, width: 1536, height: 1024, alt: dated.title }] } }; }
...
export default async function Post({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if ((aug23Meta as Record<string, unknown>)[slug]) { const x=(aug23Meta as Record<string, {title:string;description:string;hero:string}>)[slug]; const fs=require('node:fs'); const path=require('node:path'); const raw=fs.readFileSync(path.join(process.cwd(),'content/blog',`${slug}.md`),'utf8') as string; const body=raw.replace(/^---[\s\S]*?---\s*/,'').replace(/^#[^\n]+\n+/,'').replace(/^\*August 23, 2026\*\s*/,''); const blocks=body.split(/\n\n+/).filter(Boolean).map((part:string,i:number)=>part.startsWith('## ')?<h2 key={i}>{part.slice(3)}</h2>:part.startsWith('- ')?<ul key={i}>{part.split('\n').map((line:string,j:number)=><li key={j}>{line.replace(/^- /,'')}</li>)}</ul>:<p key={i}>{part}</p>); const canonical=`https://callcenteroffshore.com/blog/${slug}`; const schema={'@context':'https://schema.org','@type':'Article',headline:x.title,description:x.description,datePublished:'2026-08-23',dateModified:'2026-08-23',mainEntityOfPage:canonical,image:`https://callcenteroffshore.com${x.hero}`}; return <><Header/><main className="article-shell"><article><script type="application/ld+json" dangerouslySetInnerHTML={{__html:JSON.stringify(schema)}}/><p className="eyebrow">Blog</p><h1>{x.title}</h1><p className="lead">{x.description}</p><p><time dateTime="2026-08-23">August 23, 2026</time></p><img src={x.hero} alt={`${x.title} editorial illustration`} width="1536" height="1024"/><div className="article-body">{blocks}</div></article><CTA/></main><Footer/></>; }
  const p = blogPosts.find(x => x.slug === slug);
  if (!p) notFound();
